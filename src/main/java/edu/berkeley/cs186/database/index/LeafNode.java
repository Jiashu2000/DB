...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext() && keys.size() < Math.ceil(2 * d * fillFactor)) {
            Pair<DataBox, RecordId> next = data.next();
            DataBox key = next.getFirst();
            RecordId page = next.getSecond();
            if (keys.contains(key)) {
        ...
        if (data.hasNext()) {
            ...
            int maxSpace = (int) Math.ceil(2 * d * fillFactor);
            List<DataBox> leftKeys = keys.subList(0, maxSpace);
            List<DataBox> rightKeys = keys.subList(maxSpace, maxSpace+1);

            List<RecordId> leftRecords = rids.subList(0, maxSpace);
            List<RecordId> rightRecords = rids.subList(maxSpace, maxSpace+1);
            ...
            Pair pushedPair = new Pair<>(rightKeys.get(0), pageNum);
            return Optional.of(pushedPair);
        }
        sync();
        return Optional.empty();
    }
