...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        int nextChild = numLessThanEqual(key, keys);
        return getChild(nextChild).get(key);
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) throws BPlusTreeException {
        ...
        Optional<Pair<DataBox, Long>> r = getChild(index).put(key, rid);
        if (r.isPresent()) {
            DataBox newKey = r.get().getFirst();
            Long newPage = r.get().getSecond();
            ...
            DataBox pushedKey = innerKeys.remove(0);
            InnerNode newNode = new InnerNode(metadata, bufferManager, innerKeys, innerChildren, treeContext);
            long pageNum = newNode.getPage().getPageNum();
            newNode.sync();
            sync();
            return Optional.of(new Pair<>(pushedKey, pageNum));
        }
        sync();
        return Optional.empty();
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        if (data.hasNext()) {
            ...
            if (middle.isPresent()) {
                ...
                int i = numLessThanEqual(pushedKey, keys);
                keys.add(i, pushedKey);
                children.add(i + 1, pushedPage);
                DataBox uploadKey = keys.get(d);
                List<DataBox> leftKeys = keys.subList(0, d);
                List<DataBox> rightKeys = keys.subList(d + 1, 2 * d + 1);

                List<Long> leftChild = children.subList(0, d + 1);
                ...
                Long pageNum = newNode.getPage().getPageNum();
                Pair pushingPair = new Pair<>(uploadKey, pageNum);
                sync();
